...
export default function Home() {
  ...
  const latest = data[data.length - 1] || { temp: 0, hum: 0, air: 0 };
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-orange-500 to-red-600 p-8 rounded-[2rem] shadow-2xl">
            <p className="opacity-80 text-xs font-bold uppercase tracking-widest text-white">Temperature</p>
            <h2 className="text-5xl font-black mt-2 text-white">{Number(latest.temp).toFixed(1)}°C</h2>
          </div>
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-8 rounded-[2rem] shadow-2xl">
            <p className="opacity-80 text-xs font-bold uppercase tracking-widest text-white">Humidity</p>
            <h2 className="text-5xl font-black mt-2 text-white">{Number(latest.hum).toFixed(0)}%</h2>
          </div>
        </div>
